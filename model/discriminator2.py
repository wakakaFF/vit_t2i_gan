...
from model.vit_discriminator import ViT_D
...
class Discriminator_2(nn.Module):
    def __init__(self, modelConfig):
        ...
        self.multi_works = MultiWorkReport(modelConfig)
        self.vit_d = ViT_D()

    def forward(self, report, img):
        latent_img = self.encoder(img)

        works = self.multi_works(report, latent_img)

        out = self.vit_d(works)
